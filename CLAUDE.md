...
| `poly_force.py` | `calc_field_from_poly()` — 对每电极基做 3D 多项式拟合，以解析梯度替代格点插值得全局光滑力场；`main.py --poly-fit N` 启用并指定总次数（正整数，项数 C(N+3,3)；不带值默认 4；不传则用格点插值）；`--poly-symmetry x,z` 在拟合前剔除所列轴奇次单项式，强制关于阱中心镜面对称（与次数正交，可选任意 x/y/z 子集） |
...
| 文件 | 关键内容 |
|------|---------|
| `potential_fit_3d.py` | `fit_potential_3d_quartic()` — 3D 多项式拟合，形式由两个正交维度决定：`fit_mode`（非负整数 N=总次数≤N 基 C(N+3,3) 项，如 4→35/6→84）+ `symmetry_axes`（x/y/z 子集，拟合前剔除对称轴奇次项，强制关于 center_um 镜面对称）；`grad_fit_3d()`, `hessian_fit_3d()`，系数数组与求值/梯度/Hessian 均按实际次数动态 |
...
| `fitting.py` | `load_radial_grid_csv`（Comsol `%` 元数据行剥离 + Length unit 单位自动换算到 µm/表头列名别名/无表头 3 列/UTF-8 BOM/NaN·Inf 行自动剔除告警）+ `fit_rf_ab`（Laplace 四极+十六极基线性最小二乘，常数吸收零点；`fit_range_um` 限域去高阶泄漏；非有限值输入显式报错）→ `FitABResult`（A,B,V₀ + n_points/R²/rms/范围诊断）；`--fit-ab` 后端 |
