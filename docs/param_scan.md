...
终止于 **σ_y < 0.0005µm** 或模拟达 **50µs**，存结束位置 `.npy`、维护 `scan.log`，
...
- **复用动力学基础设施**：`main.py::_create_backend_and_start` 启动 backend 子进程，
  backend 在 `time=50µs` **自动 STOP**；σ_y 达标则由本模块消费者提前 STOP。`main.py`/`backend.py` 不改动。
...
`CONFIG_JSON`/`TIME_MAX_US`/`SIGMA_Y_THRESH_UM`/`INIT_RANGE_UM`/`GAMMA`/`STEP`/`INTERVAL`/`BATCH`）。

### 2) 热力图（与动力学解耦）

```bash
python -m param_scan plot --log param_scan/results/scan.log --out heat.png
# 颜色列可选 sigma_y_final_um / sim_time_us / reached_threshold；σ_y 跨数量级时加 --log-color
...
| `seed` | 当轮随机种子 |
| `reached_threshold` | 1/0，50µs 内是否 σ_y<阈值 |
| `sim_time_us` | 达标→跨越时刻；未达标→50.0 |
