...
class Configure:
    ...
    def simulation(self, N: int, ini_range: int, mass: np.ndarray, charge: np.ndarray, step: int, interval: int, batch: int, t: float, device: bool, plotting: bool, alpha: float = 1.0, 
        ...
        mask_final = (np.abs(f.r[:, 0]*self.dl*1e6)<50) & (np.abs(f.r[:, 1]*self.dl*1e6)<250) & (np.abs(f.r[:, 1]*self.dl*1e6)>100) & (np.abs(f.r[:, 2]*self.dl*1e6)<200) if bilayer else (np.abs(f.r[:, 0]*self.dl*1e6)<100) & (np.abs(f.r[:, 1]*self.dl*1e6)<50) & (np.abs(f.r[:, 2]*self.dl*1e6)<1000)
