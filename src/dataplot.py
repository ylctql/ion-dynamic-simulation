...
class DataPlotter:
	def __init__(self, queue_in: mp.Queue, queue_out: mp.Queue, frame_init : Frame, interval: float, x_range=50, y_range=50, z_range=50, x_bias=0, y_bias=0, z_bias=0, dl=1, dt: float = 1, bilayer: bool = False):
		...
		if self.bilayer:
			# bilayer
			self.ax1 = plt.subplot2grid((6, 6), (0, 0), colspan=1, rowspan=6, fig=self.fig)
			self.ax2 = plt.subplot2grid((6, 6), (0, 1), colspan=1, rowspan=6, fig=self.fig)
			self.ax3 = plt.subplot2grid((6, 6), (0, 2), colspan=1, rowspan=6, fig=self.fig)
			self.ax4 = plt.subplot2grid((6, 6), (0, 3), colspan=3, rowspan=1, fig=self.fig)
			self.ax5 = plt.subplot2grid((6, 6), (1, 3), colspan=3, rowspan=1, fig=self.fig)
			self.ax6 = plt.subplot2grid((6, 6), (2, 3), colspan=3, rowspan=4, fig=self.fig)

			self.ax1.set_ylim(-z_range+z_bias, z_range+z_bias)
			self.ax1.set_xlim(-x_range+x_bias, x_range+x_bias)
			self.ax1.set_aspect('equal')
			self.ax1.set_ylabel('z/um', fontsize=14)
			self.ax1.set_xlabel('x/um', fontsize=14)
			self.ax1.tick_params(axis='x', labelsize=14)
			self.ax1.tick_params(axis='y', labelsize=14)
			self.ax1.set_title("Upper layer", fontsize=14)
			...
			self.ax2.set_title("Lower layer", fontsize=14)

			self.ax3.set_ylim(-z_range+z_bias, z_range+z_bias)
			self.ax3.set_xlim(-x_range+x_bias, x_range+x_bias)
			self.ax3.set_aspect('equal')
			self.ax3.set_ylabel('z/um', fontsize=14)
			self.ax3.set_xlabel('x/um', fontsize=14)
			self.ax3.tick_params(axis='x', labelsize=14)
			self.ax3.tick_params(axis='y', labelsize=14)
			self.ax3.set_title("Bilayer", fontsize=14)

			self.ax4.set_xlim(-z_range+z_bias, z_range+z_bias)
			self.ax4.set_ylim(y_range-100+y_bias, y_range+y_bias)
			self.ax4.set_aspect('equal')
			self.ax4.set_xlabel('z/um', fontsize=14)
			self.ax4.set_ylabel('y/um', fontsize=14)
			self.ax4.tick_params(axis='x', labelsize=14)
			self.ax4.tick_params(axis='y', labelsize=14)
			self.ax4.set_title("Upper layer", fontsize=14)

			self.ax5.set_xlim(-z_range+z_bias, z_range+z_bias)
			self.ax5.set_ylim(-y_range+y_bias, -y_range+100+y_bias)
			self.ax5.set_aspect('equal')
			self.ax5.set_xlabel('z/um', fontsize=14)
			self.ax5.set_ylabel('y/um', fontsize=14)
			self.ax5.tick_params(axis='x', labelsize=14)
			self.ax5.tick_params(axis='y', labelsize=14)
			self.ax5.set_title("Lower layer", fontsize=14)

			self.ax6.set_xlim(-z_range+z_bias, z_range+z_bias)
			self.ax6.set_ylim(-y_range+y_bias, y_range+y_bias)
			self.ax6.set_aspect('equal')
			self.ax6.set_xlabel('z/um', fontsize=14)
			self.ax6.set_ylabel('y/um', fontsize=14)
			self.ax6.tick_params(axis='x', labelsize=14)
			self.ax6.tick_params(axis='y', labelsize=14)

			self.artists = (
			self.ax1.scatter(frame_init.r[frame_init.r[:, 1] > 0, 0]*self.dl, frame_init.r[frame_init.r[:, 1] > 0, 2]*self.dl, 5, 'b'),
			self.ax2.scatter(frame_init.r[frame_init.r[:, 1] < 0, 0]*self.dl, frame_init.r[frame_init.r[:, 1] < 0, 2]*self.dl, 5, 'r'),
			self.ax3.scatter(frame_init.r[:, 0]*self.dl, frame_init.r[:, 2]*self.dl, 5, 'r'),
			self.ax4.scatter(frame_init.r[frame_init.r[:, 1] > 0, 2]*self.dl, frame_init.r[frame_init.r[:, 1] > 0, 1]*self.dl, 5, 'b'),
			self.ax5.scatter(frame_init.r[frame_init.r[:, 1] < 0, 2]*self.dl, frame_init.r[frame_init.r[:, 1] < 0, 1]*self.dl, 5, 'r'),
			self.ax6.scatter(frame_init.r[:, 2]*self.dl, frame_init.r[:, 1]*self.dl, 5, 'r'),
		)
		else:
	...
	def plot(self):
		...
		if self.bilayer:
			# bilayer
			self.artists[0].set_offsets(np.vstack((f.r[f.r[:, 1] > 0, 0]*self.dl, f.r[f.r[:, 1] > 0, 2]*self.dl)).T)
			self.artists[1].set_offsets(np.vstack((f.r[f.r[:, 1] < 0, 0]*self.dl, f.r[f.r[:, 1] < 0, 2]*self.dl)).T)
			self.artists[2].set_offsets(np.vstack((f.r[:, 0]*self.dl, f.r[:, 2]*self.dl)).T)
			self.artists[3].set_offsets(np.vstack((f.r[f.r[:, 1] > 0, 2]*self.dl, f.r[f.r[:, 1] > 0, 1]*self.dl)).T)
			self.artists[4].set_offsets(np.vstack((f.r[f.r[:, 1] < 0, 2]*self.dl, f.r[f.r[:, 1] < 0, 1]*self.dl)).T)
			self.artists[5].set_offsets(np.vstack((f.r[:, 2]*self.dl, f.r[:, 1]*self.dl)).T)
			# Bilayer: +为蓝色，-为红色
			colors = np.full(f.r.shape[0], 'r')
			colors[f.r[:,1]>0] = 'b'
			self.artists[2].set_facecolor(colors)
			self.artists[5].set_facecolor(colors)
			self.ax6.set_title("timestamp=%.2f, t=%.3fus"%(f.timestamp,f.timestamp*self.dt), fontsize=14)
		else:
