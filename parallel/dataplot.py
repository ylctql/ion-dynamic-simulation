...
class DataPlotter:
	def __init__(self, queue_in: mp.Queue, queue_out: mp.Queue, frame_init : Frame, interval: float, gamma=0.1, x_range=50, y_range=50, z_range=50, x_bias=0, y_bias=0, z_bias=0, dl=1, dt=1):
		...
		self.fig, self.ax = plt.subplots(2, 1, figsize=(20, 10), gridspec_kw={'height_ratios': [1, 5]})
		...
		self.prefix = '../data_cache/60electrodes_tiny/'
		...
		self.artists = (
			...
			self.ax[1].scatter(frame_init.r[:, 2]*self.dl, frame_init.r[:, 0]*self.dl, 10, 'r'),
		)
	...
	def plot(self):
		...
		if f.timestamp*self.dt>20:
			self.last_loss = self.LossFunction(f.r)
			return False
	...
	def start(self,):
		...
		plt.close(self.fig)
		# return self.LossFunction(self.last_loss)
