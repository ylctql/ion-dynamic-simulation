...
| `--save_times_us` | - | 需保存轨迹图的时刻 (μs)：逗号分隔如 10,20,30；或 **`start:stop:step`**（同 Python `range`，stop 不含），如 `100:1100:100` → 100…1000，**无括号、bash 可直接写**；或 `'range(100,1100,100)'`（须引号，否则 shell 把 `(` 当语法）；可混写；无窗口，仅保存图片 |
